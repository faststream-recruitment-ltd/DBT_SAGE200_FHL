{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_fhl.nominal_codes_financial_years_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_fhl.nominal_codes_financial_years_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- nominal_codes_financial_years from {{ source('sage200_etl_fhl', '_airbyte_raw_nominal_codes') }}
    where 1 = 1
