{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_fhl.suppliers_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_fhl.suppliers_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- suppliers from {{ source('sage200_etl_fhl', '_airbyte_raw_suppliers') }}
    where 1 = 1
