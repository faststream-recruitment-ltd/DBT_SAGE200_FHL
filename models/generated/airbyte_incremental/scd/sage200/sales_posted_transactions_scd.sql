{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_fhl.sales_posted_transactions_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_fhl.sales_posted_transactions_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- sales_posted_transactions from {{ source('sage200_etl_fhl', '_airbyte_raw_sales_posted_transactions') }}
    where 1 = 1
