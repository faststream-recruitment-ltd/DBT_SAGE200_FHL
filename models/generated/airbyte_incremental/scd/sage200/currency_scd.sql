{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_fhl.currency_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_fhl.currency_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- currency from {{ source('sage200_etl_fhl', '_airbyte_raw_currency') }}
    where 1 = 1
