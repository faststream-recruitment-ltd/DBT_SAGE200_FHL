{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_fhl.nominal_transaction_enquiry_views_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_fhl.nominal_transaction_enquiry_views_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- nominal_transaction_enquiry_views from {{ source('sage200_etl_fhl', '_airbyte_raw_nominal_transaction_enquiry_views') }}
    where 1 = 1
