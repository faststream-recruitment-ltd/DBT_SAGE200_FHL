{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_fhl.cost_centres_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_fhl.cost_centres_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- cost_centres from {{ source('sage200_etl_fhl', '_airbyte_raw_cost_centres') }}
    where 1 = 1
