{{ config(
    ...
    schema = "_airbyte_sage200_etl_fhl",
    tags = [ "top-level-intermediate" ]
) }}
-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: {{ source('sage200_etl_fhl', '_airbyte_raw_nominal_transaction_enquiry_views') }}
select
...
from {{ source('sage200_etl_fhl', '_airbyte_raw_nominal_transaction_enquiry_views') }} as table_alias
